...
//                 onClick={() => setActiveReplyDiscussionId(discussion.id)}
//               >
//                 <RiReplyLine size={20} />
//                 Reply
//               </button>
...
const Discussions = () => {
  ...
  const handleSubmitReply = () => {
    if (replyContent.trim()) {
      console.log("New reply:", replyContent);
      setReplyContent("");
      setActiveReplyDiscussionId(null); // Hide after submit
    }
  };

  const handleCancelReply = () => {
    setReplyContent("");
    setActiveReplyDiscussionId(null); // Hide on cancel
  };
      ...
      <div className={styles.discussionsList}>
        {discussions.map((discussion) => (
          <div key={discussion.id}>
            ...
            {/* Reply Editor (conditionally visible) */}
            {activeReplyDiscussionId === discussion.id && (
              <div className={styles.replyEditor}>
                <div className={styles.replyInputContainer}>
                  <Image
